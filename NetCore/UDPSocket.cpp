...
void UDPSocket::recieve(uint8* buffer, uint32 bufferSize) {
	// TODO in the future replace with Packet class
	sockaddr senderAddress;
	int32 senderAddressSize = sizeof(senderAddress);

	int32 numBytesRecieved = recvfrom(_winSocket, reinterpret_cast<char*>(buffer), bufferSize, 0, &senderAddress, &senderAddressSize);
}